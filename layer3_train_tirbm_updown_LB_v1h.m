function layer3_train_tirbm_updown_LB_v1h(images_all, ws, num_bases, pbias, pbias_lb, pbias_lambda, spacing, epsilon, l2reg, batch_size,vis_layer2)
...
batch_ws = 15; % changed from 100 (2008/07/24)
imbatch_size = numel(images_all);

fname_prefix = sprintf('../results/tirbm/layer3_tirbm_updown_LB_new1h_w%d_b%02d_p%g_pl%g_plambda%g_sp%d_CD_eps%g_l2reg%g_bs%02d_%s', ws, num_bases, pbias, pbias_lb, pbias_lambda, spacing, epsilon, l2reg, batch_size, datestr(now, 30));
...
%              rowidx = ceil(rand*(rows-2*ws-batch_ws)) + [1:batch_ws];
%   rowidx=ceil(randi([1,9])) + [1:batch_ws];
%              colidx = ceil(rand*(cols-2*ws-batch_ws)) + [1:batch_ws];
%   colidx=ceil(randi([1,9])) + [1:batch_ws];
 rowidx=[1:batch_ws];
 colidx=[1:batch_ws];
...
figure(1), display_network_layer2(W,vis_layer2);
     if mod(t,1)==0,
...
%      save('layer3.mat', 'W', 'pars', 't', 'vbias_vec', 'hbias_vec', 'error_history', 'sparsity_history');
